...
RawModel loadOBJToModel(const char* fileName, Loader& loader)
{
    ...
    RawModel model = loader.loadToVAO(modelData.getMeshes()[0].vertices, modelData.getMeshes()[0].texCoords, modelData.getMeshes()[0].normals, modelData.getMeshes()[0].indices);
    modelData.cleanUp();
    return model;
}
